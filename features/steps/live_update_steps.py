...
async def create_item(async_client, context):
    response = await async_client.post("/items", data={"name": "Live Update Item"})
...
@pytest.mark.asyncio
async def connect_updates_short_timeout(async_client, context, polling_task):
    async def poll():
        response = await async_client.get("/updates/poll?timeout=0.1")
        return response

    context["response"] = asyncio.create_task(poll())
    await asyncio.sleep(0.1)  # Let the connection establish


@when("I wait for the timeout period")
@pytest.mark.asyncio
async def wait_for_timeout(async_client, context):
    await asyncio.sleep(0.1)


@then("I should receive a timeout response")
async def check_timeout(context):
